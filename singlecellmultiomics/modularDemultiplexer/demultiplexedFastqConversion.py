...
if __name__=='__main__':
    argparser = argparse.ArgumentParser(
     ...
     """) # -o ./bc_umi.fastq.gz
    argparser.add_argument('-seqfeatures',type=str, help="Features to put into the ", required=True)
    argparser.add_argument('-qualityfeatures',type=str, help="Features to put into ", required=True)
    argparser.add_argument('fastqfiles', nargs='*',   type=str, help="Input demultiplexed fastq files")
    ...
    for reads in FastqIterator(*args.fastqfiles):
        for readIndex, record in enumerate(reads):
            ...
            if len(sequence)!=len(qualities):
                raise ValueError('Length of base qualities does not match the length of the selected sequence features')
