...
class Base_RestrictionBisulfiteDemuxMethod(UmiBarcodeDemuxMethod):
	...
	def __init__(self,
		...
		**kwargs ): # additional arguments
		...
		umiRead=umiRead, umiStart = umiStart, umiLength=umiLength,
		barcodeRead=barcodeRead, barcodeStart = barcodeStart, barcodeLength=barcodeLength,
		...
		self.illumina_mux = IlluminaBaseDemultiplexer(indexFileParser=indexFileParser,indexFileAlias='illumina_merged_ThruPlex48S_RP')
		...
		if umiLength==0:
			...
			if not( umiStart==0 or barcodeStart==0 ):
				raise NotImplementedError('Complicated slice where we need to capture around a region')
			self.sequenceCapture[barcodeRead] = 	slice( barcodeLength+umiLength+enzymeLength+ispcrLength, None)
	...
	def demultiplex(self, records, **kwargs):
		...
		try:
			taggedRecords = self.illumina_mux.demultiplex( records, inherited=True,  **kwargs)
		...
		if self.enzymeLength!=0:
			enz = records[self.enzymeRead].sequence[self.enzymeStart:self.enzymeStart+self.enzymeLength]
			enzQual = records[self.enzymeRead].qual[self.enzymeStart:self.enzymeStart+self.enzymeLength]

		if self.ispcrLength!=0:
			ispcr = records[self.ispcrRead].sequence[self.ispcrStart:self.ispcrStart+self.ispcrLength]
			ispcrQual = records[self.ispcrRead].qual[self.ispcrStart:self.ispcrStart+self.ispcrLength]
...
class Nla_384w_u8_c8_ad3_is15(Base_RestrictionBisulfiteDemuxMethod):
	def __init__(self, barcodeFileParser,  **kwargs ):
		...
		Base_RestrictionBisulfiteDemuxMethod.__init__(self,
	 	barcodeFileAlias = self.barcodeFileAlias,
		barcodeFileParser = barcodeFileParser,
		**kwargs)
		self.shortName = 'ReBsNla384C8U8E3I15'
