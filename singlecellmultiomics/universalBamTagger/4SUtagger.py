#!/usr/bin/env python3
import pysam
from singlecellmultiomics.molecule import FourThiouridine
...
argparser.add_argument('-ref', type=str, help='reference (autodetected if not supplied)')
...
converted_base_frequencies = collections.defaultdict( lambda: collections.defaultdict( collections.Counter ))

with pysam.AlignmentFile(args.bamfile) as alignments:

    with singlecellmultiomics.bamProcessing.sorted_bam_file(f'{args.bamfile}.4SU_tagged.bam',origin_bam=alignments) as out:
        test_iter = singlecellmultiomics.molecule.MoleculeIterator(alignments,
                fragment_class_args=fragment_class_args,
                molecule_class_args={ 'reference':reference},
               moleculeClass=FourThiouridine,check_eject_every=1000,
                pooling_method=0,
