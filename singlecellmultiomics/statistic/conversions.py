...
class ConversionMatrix(StatisticHistogram):
    def __init__(self,args, process_reads=200_000):
        ...
        self.stranded_base_conversions = collections.defaultdict(collections.Counter)
    ...
    def processRead(self,read):
        ...
        for index, reference_pos, reference_base in read.get_aligned_pairs(with_seq=True,matches_only=True):
            query_base = read.seq[index]
            reference_base = reference_base.upper()
            if reference_base!='N' and query_base!='N':
                k = (query_base, 'R1' if read.is_read1 else ( 'R2' if read.is_read2 else 'R?' ), ('forward','reverse')[read.is_reverse])
                ...
                if reference_base!=query_base:
                    ...
                    if read.has_tag('RS'):
                        k = (query_base, 'R1' if read.is_read1 else ( 'R2' if read.is_read2 else 'R?' ), read.get_tag('RS'))
    ...
    def get_df(self):
        return pd.DataFrame(self.base_obs).fillna(0).sort_index(axis=0).sort_index(axis=1)

    def plot(self, target_path, title=None):

        df = pd.DataFrame(self.conversion_obs).fillna(0).sort_index(axis=0).sort_index(axis=1)
        ...
        df = pd.DataFrame(self.stranded_base_conversions).fillna(0).sort_index(axis=0).sort_index(axis=1)
        cm = sns.clustermap(df,row_cluster=False, col_cluster=False)
        ax = cm.ax_heatmap
        ax.set_yticklabels( ax.get_yticklabels(), rotation=0)
        cm.ax_heatmap.set_title('Raw conversions')
        ...
        plt.savefig(target_path.replace('.png',f'.RS_TAG_strand_conversions.png'))
        plt.close()
