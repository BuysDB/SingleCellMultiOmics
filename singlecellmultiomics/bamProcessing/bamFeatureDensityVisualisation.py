...
def distance_to_feature_start(chromosome,lookup_coordinate,feature_container,lookup_strand=None):
    ...
    for hit in feature_container.findFeaturesAt(chromosome=chromosome,lookupCoordinate=lookup_coordinate,strand=lookup_strand):
        ...
        distance = distance_to_hit( lookup_coordinate, hit_start, hit_end, hit_strand, lookup_strand)
        if min_distance is None or abs(distance)<abs(min_distance):
            min_distance = distance
    if distance is None:
        for hit in feature_container.findNearestFeature(chromosome=chromosome,lookupCoordinate=lookup_coordinate,strand=lookup_strand):
            hit_start, hit_end, hit_id, hit_strand, hit_ids = hit
