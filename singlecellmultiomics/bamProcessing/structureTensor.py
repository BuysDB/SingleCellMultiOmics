...
argparser.add_argument('-sample_non_variable_per_contig',  type=int, default=2, help='Select this amount of non-variable bases per contig')
...
if args.contig is None:
    try:
        contigs_per_job = 50
        contigs_in_job = []
        for i,stat in enumerate(alignments_positive.get_index_statistics()):
            if stat.mapped < args.min_coverage:
                continue
            contig = stat.contig
            contigs_in_job.append(contig)
            if len(contigs_in_job)>=contigs_per_job:
                arguments = " ".join([x for x in sys.argv])  + f" -contig {','.join(contigs_in_job)}"
                job = f'STRUCT_{i}'
    ...
    for contig in args.contig.split(','):
        ...
        for column in alignments_positive.pileup(contig=contig):
            ...
            if column.nsegments>=args.min_coverage:
                ...
                if edits.most_common(1)[0][1]<(1-args.mindiff)*sum(edits.values()):
            ...
            for query_pos, ref_pos, ref_base in read.get_aligned_pairs(with_seq=True):
                if ref_pos is None: # clip or skip
                    ...
                    if query_base!='N':
                        ...
                        conversions[(contig,ref_pos)][f'{ref_base}>{query_base}'] += 1
        ...
        X = []
        positions = []
        features_to_plot = ['insert', 'deleted','match', 'mismatch', 'clip']
        for location in variable_locations:
            radius = args.radius
            features = df.loc[contig].loc[location-radius:location+radius]
        ...
        if len(non_variant_enough_cov)>0:
            for location in random.sample(non_variant_enough_cov, min(args.sample_non_variable_per_contig, len(non_variant_enough_cov))):
                radius = args.radius
                features = df.loc[contig].loc[location-radius:location+radius]
