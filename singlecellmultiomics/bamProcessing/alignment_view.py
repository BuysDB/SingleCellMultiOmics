...
highlight=set()
    ...
    chrom, _start_span, _end_span = pysamiterators.getListSpanningCoordinates(molecule_flattened)
        ...
        for read in [R1,R2]:
            ...
            for i,(cycle,qpos,refpos,refbase) in enumerate(it):
                ...
                if read.is_read2 and cycle<R2PrimerLength:
                    ...
                    visQuals[visPos] = cstr(html.escape(str(qqual)),color='black',weight=weight)
                    ...
                    visQuals[visPos] = cstr(html.escape(str(qqual)),color=c,weight=weight)
                    ...
                    if qbase!=refbase:
                        weight=700
                        visBases[visPos] = cstr(qbase,color='red',weight=weight)
                        weight=400
                        visQuals[visPos] = cstr(html.escape(str(qqual)),color='black',weight=weight)
                    else:
                        weight=700
                        visBases[visPos] = cstr(qbase,color='green',weight=weight)
                        weight=400
                        visQuals[visPos] = cstr(html.escape(str(qqual)),color='black',weight=weight)
                    ...
                    if qbase!=refbase:
                        weight=700
                        visBases[visPos] = cstr(f'{qbase}',color='orange',weight=weight) #+'/'+cstr(refbase,color='red',weight=weight)
                        weight=400
                        visQuals[visPos] = cstr(html.escape(str(qqual)),color='black',weight=weight)
                    else:
                        weight=100
                        visBases[visPos] = cstr(qbase,color='grey',weight=weight)
                        visQuals[visPos] = cstr(html.escape(str(qqual)),color='#CCC',weight=weight)
